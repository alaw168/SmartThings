...
metadata {
	definition(name: "SmartSense Multi Sensor-Vibration", namespace: "alaw168", author: "Alvin Law", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false, mnmn: "SmartThings", vid: "generic-contact-2") {
	...
	tiles(scale: 2) {
		multiAttributeTile(name: "acceleration", type: "generic", width: 6, height: 4) {
			tileAttribute("device.acceleration", key: "PRIMARY_CONTROL") {
				attributeState "active", label: 'Active', icon: "st.Appliances.appliances8", backgroundColor: "#00a0dc"
				attributeState "inactive", label: 'Inactive', icon: "st.Appliances.appliances1", backgroundColor: "#cccccc"
			}
		}
		...
		standardTile("status", "device.status", width: 2, height: 2) {
			state("open", label: 'Open', icon: "st.contact.contact.open", backgroundColor: "#e86d13")
			state("closed", label: 'Closed', icon: "st.contact.contact.closed", backgroundColor: "#00a0dc")
			state("garage-open", label: 'Open', icon: "st.doors.garage.garage-open", backgroundColor: "#e86d13")
			state("garage-closed", label: 'Closed', icon: "st.doors.garage.garage-closed", backgroundColor: "#00a0dc")
		}
		...
		main(["acceleration", "contact", "temperature"])
		details(["acceleration", "contact", "temperature", "battery", "refresh"])
	}
}
